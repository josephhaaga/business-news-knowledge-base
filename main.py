...
def main() -> int:
    ...
    for entity, mentions in consolidated_entities.items():
        ...
        # TODO: use SPARQL query to find matches that are of the expected NER type (or subtype)
        if len(possible_matches) < 1:
            print(f"Wikidata has no match for {entity}")
            continue
        pos_tag_description = nlp(
            spacy.explain(mentions[0].label_)
        )
        results = []
        for match in possible_matches:
            # use a Wikidata library to grab metadata/categories (description embeddings don't work)
            if len(match["description"]) > 0:
                score = nlp(match["description"]).similarity(pos_tag_description)
                results += [(match["id"], score)]
                print(f"{match['title']}: {match['description']}")
        breakpoint()
        results.sort(key=lambda x: x[1], reverse=True)
        top_result, *_ = results
        (best_match,) = [
            match for match in possible_matches if match["id"] == top_result[0]
        ]
        print(
            f"{entity} is likely {best_match['id']} - {best_match['title']}: {best_match['description']} with confidence {top_result[1]}"
        )
        print()
    ...
    return [
        {
            ...
            "id": result.find("span", class_="wb-itemlink-id")
            .text.lstrip("(")
            .rstrip(")"),
